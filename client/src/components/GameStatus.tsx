...
export function GameStatus({ showPrizePoolOnly, showTimeRemainingOnly, showLastGuessOnly, onTimerEnd }: GameStatusProps) {
  ...
  useEffect(() => {
    ...
    const initializeTime = async () => {
      try {
        ...
        const MAX_GAME_TIME = 172800; // 48 hours (172800 seconds)
  ...
  useEffect(() => {
    ...
    const updateStatus = async () => {
      try {
        ...
        const time = Number(timeRemaining.toString());
        // Cap the time to 48 hours (172800 seconds)
        const MAX_GAME_TIME = 172800; // 48 hours (172800 seconds)
  ...
  useEffect(() => {
    ...
    const handleFreshContract = (event: Event) => {
      ...
      // Reset the game timer to 48 hours (172800 seconds) for the new game
      setDisplayTime(172800);
      setBaseTime(172800);
