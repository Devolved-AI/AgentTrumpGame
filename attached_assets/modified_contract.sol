...
contract AgentTrumpGame is ReentrancyGuard, Ownable, Pausable {
    ...
    event GameForciblyEnded(address indexed owner, uint256 ownerReward);
    ...
    // New function to allow owner to end the game at any time
    function forceEndGame() external onlyOwner {
        ...
        // Mark the game as won to prevent further submissions
        gameWon = true;
        // Set game end block to current block to prevent normal endGame function
        gameEndBlock = block.number;

        emit GameForciblyEnded(owner(), totalBalance);
    }
